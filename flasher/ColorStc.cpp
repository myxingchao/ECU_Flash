...
BEGIN_MESSAGE_MAP(CColorStc, CButton)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()
//
void CColorStc::SetColor(DWORD color)
{
	m_dwColor = color;
}
void CColorStc::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	CPen	pen;
	CBrush	brush;
	if (::IsWindowEnabled(::GetParent(m_hWnd)) == FALSE || (lpDIS->itemState & ODS_DISABLED) != 0)
	{
	...
	else if ((lpDIS->itemState & ODS_SELECTED) == 0)//禁用、未高亮且未选中状态
	{
		...
		pdc->SetTextColor(HLS_TRANSFORM(m_dwColor, 90, 0));
	}
	else//选中、高亮状态(没用到)
	{
		...
		if (lpDIS->itemState & ODS_SELECTED)//选中
			...
			brush.CreateSolidBrush(HLS_TRANSFORM(m_dwColor, +90, -70));//COLOR_HIGHLIGHT
	...
	pdc->DrawText(txt, rtc, DT_SINGLELINE |DT_CENTER |DT_VCENTER);
...
/*void CColorStc::DrawXPTheme(LPDRAWITEMSTRUCT lpDIS) 
{
	CDC* pdc = CDC::FromHandle(lpDIS->hDC);
}*/
/*void CColorStc::DrawXPTheme(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
