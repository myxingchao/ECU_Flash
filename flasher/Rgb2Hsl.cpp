	...
	if ( (hDC = ::GetDC(NULL)) != NULL )
	{
		if ( (hMemDC = ::CreateCompatibleDC(hDC)) != NULL )
		{
			if ( (hNewBitmap = ::CreateCompatibleBitmap(hDC, pRect->right - pRect->left, pRect->bottom - pRect->top)) != NULL )
			{
				...
				::BitBlt(hMemDC, 0, 0, pRect->right - pRect->left, pRect->bottom - pRect->top, hDC, pRect->left, pRect->top, SRCCOPY);
...
BOOL CPHDraw::DrawVerticalText(CDC* pDC,LPCRECT lpRect,LPCTSTR lpText,int length,UINT format,LOGFONT *lf)
{
...
BOOL CPHDraw::DrawBitmap(CDC* pDC, LPCRECT lpRect, CBitmap* pBitmap,UINT Mode)
{
...
HRGN CPHDraw::BitmapToRegion(HBITMAP hBmp, COLORREF cTransparentColor, COLORREF cTolerance)
{
	HRGN hRgn = NULL;
	if (hBmp)
	{
...
BOOL CPHDraw::FillGradient(CDC *pDC, CRect rect, COLORREF colorStart, COLORREF colorFinish, BOOL bHorz /*= TRUE*/ )
{
	int nShift = 6;
	int nSteps = 1 << nShift;
...
void CPHDraw::DrawShadow(HDC hDCIn, HDC hDCOut, RECT& rc)
{
