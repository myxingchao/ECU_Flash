...
// Copyright (c) 2001
//
// To use CPicture, follow these steps:
...
//   - in your dialog's header file replace CStatic with CPicture
//     (don't forget to #include "Picture.h" and add 
//     Picture.h and Picture.cpp to your project)
//   - call one of the overloaded CPicture::Load() functions somewhere
...
// CPicture, calling CPicture::Create (derived from CStatic), then 
// CPicture::Load and CPicture::Draw.
//
// By default, the control initializes its background to COLOR_3DFACE
// (see CPicture::PrepareDC()). You can change the background by
// calling CPicture::SetBkColor(COLORREF) after CPicture::Load().
//
...
// and an appropriate section in CPicture::Load(HGLOBAL, DWORD). These functions 
...
//                Now CPicture[Wnd] works in Win95 too.
...
//                CPictureWnd::PrepareDC() function).
//
//              - Now it is possible to set a clipping rectangle using
//                CPicture[Wnd]::SetPaintRect(const LPRECT) function.
...
inline int CPicture::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int CPicture::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int CPicture::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BOOL CPicture::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL CPicture::Draw()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CPicture::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
...
BOOL CPicture::Load(LPCTSTR szFileName)
{
	ASSERT(szFileName);
...
BOOL CPicture::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
void CPicture::ResetDataPointer()
{
...
BOOL CPicture::SkipNextGraphicBlock()
{
...
UINT CPicture::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum CPicture::GIFBlockTypes CPicture::GetNextBlock() const
{
...
int CPicture::GetNextBlockLen() const
{
...
UINT WINAPI CPicture::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	CPicture *pPic = reinterpret_cast<CPicture *> (pParam);
...
HGLOBAL CPicture::GetNextGraphicBlock(UINT *pBlockLen, 
...
BOOL CPicture::IsGIF() const
{
	return m_bIsGIF;
}

BOOL CPicture::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL CPicture::IsPlaying() const
{
	return m_bIsPlaying;
}

int CPicture::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF CPicture::GetBkColor() const
{
...
BOOL CPicture::PrepareDC(int nWidth, int nHeight)
{
...
void CPicture::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void CPicture::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
BOOL CPicture::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL CPicture::GetPaintRect(RECT *lpRect)
{
